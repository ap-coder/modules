...
<div class="card">
    <div class="card-header">
        {{ trans('mcode::global.edit') }} {{ trans('mcode::cruds.mcode.title_singular') }}
    </div>
        ...
        <form method="POST" action="{{ route("admin.mcodes.update", [$mcode->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('published') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="published" id="published" value="1" {{ $mcode->published || old('published', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $mcode->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('models') ? 'is-invalid' : '' }}" name="models[]" id="models" multiple>
                    @foreach($models as $id => $models)
                        <option value="{{ $id }}" {{ (in_array($id, old('models', [])) || $mcode->models->contains($id)) ? 'selected' : '' }}>{{ $models }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $mcode->slug) }}">
                @if($errors->has('slug'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('desc') ? 'is-invalid' : '' }}" name="desc" id="desc">{!! old('desc', $mcode->desc) !!}</textarea>
                @if($errors->has('desc'))
            ...
            <div class="form-group">
              ...
              <input class="form-control {{ $errors->has('order') ? 'is-invalid' : '' }}" type="number" name="order" id="order" value="{{ old('order', $mcode->order) }}" step="1">
              @if($errors->has('order'))
