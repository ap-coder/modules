...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.mcodes.update", [$mcode->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="product">{{ trans('cruds.mcode.fields.product') }}</label>
                <input class="form-control {{ $errors->has('product') ? 'is-invalid' : '' }}" type="text" name="product" id="product" value="{{ old('product', $mcode->product) }}">
                @if($errors->has('product'))
                    <span class="text-danger">{{ $errors->first('product') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.mcode.fields.product_helper') }}</span>
            </div>
            <div class="form-group">
