...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.mcode-features.update", [$mcodeFeature->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="product_models">{{ trans('mcode::cruds.mcodeFeature.fields.product_models') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('product_models') ? 'is-invalid' : '' }}" name="product_models[]" id="product_models" multiple>
                    @foreach($product_models as $id => $product_models)
                        <option value="{{ $id }}" {{ (in_array($id, old('product_models', [])) || $mcodeFeature->product_models->contains($id)) ? 'selected' : '' }}>{{ $product_models }}</option>
                    @endforeach
                </select>
                @if($errors->has('product_models'))
                    <span class="text-danger">{{ $errors->first('product_models') }}</span>
                @endif
                <span class="help-block">{{ trans('mcode::cruds.mcodeFeature.fields.product_models_helper') }}</span>
            </div>
            <div class="form-group">
