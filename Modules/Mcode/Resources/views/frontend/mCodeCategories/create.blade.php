        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.create') }} {{ trans('cruds.mCodeCategory.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.m-code-categories.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="compatible_products[]" id="compatible_products" multiple>
                                @foreach($compatible_products as $id => $compatible_products)
                                    <option value="{{ $id }}" {{ in_array($id, old('compatible_products', [])) ? 'selected' : '' }}>{{ $compatible_products }}</option>
                                @endforeach
                            </select>
