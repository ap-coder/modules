<div class="mcode_step_holder category_holder">
    <h2 class="text-4" style=" font-size: 1.2rem!important;">What type of category are you looking for?</h2>
    ...
    <div class="category_box">
        @foreach ($categories as $category)
        <div class="mcode-category">
            <div class="mcode-cat-name">{{ $category->name }}</div>
            <div class="mcode-cat-select">
                <label class="checkbox">
                    <input type="checkbox" name="category" id="category" class="checkboxinput" value="{{ $category->id }}"/>
                    <span class="primary"></span>
                </label>
